/**
 * Angular Filter: timeAgo
 * 
 * Given a timestamp (seconds since 1970) returns a time ago string. For example:
 * 
 * 		"less than a minute ago"
 * 		"about an hour ago"
 * 		"2 years ago"
 * 
 * This filter can be used both declaratively in HTML like this:
 * 		<span>{{ someTimestamp | timeAgo }}</span>
 * 
 * -- or imperatively in javascript via a function call like this:
 * 		var myTimeAgo = $filter('timeAgo')(new Date().getTime());
 */
  ...
  app.filter('timeAgo', function () {
    return function (input, p_allowFuture) {
      ...
      var substitute = function (stringOrFunction, number, strings) {
        ...
        }, nowTime = new Date().getTime(), date = new Date(input).getTime(),
        ...
        allowFuture = p_allowFuture || false, strings = {
          ...
          year: 'about a year',
          years: '%d years'
        }, dateDifference = nowTime - date, words, seconds = Math.abs(dateDifference) / 1000, minutes = seconds / 60, hours = minutes / 60, days = hours / 24, years = days / 365, separator = strings.wordSeparator === undefined ? ' ' : strings.wordSeparator, prefix = strings.prefixAgo, suffix = strings.suffixAgo;
      if (allowFuture) {
        if (dateDifference < 0) {
